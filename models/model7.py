...
def get_spatio_temporal_edges(T=8, N=256, K=8):
    ...
    Precompute ONCE (static for all videos):
      ...
      - Temporal exact-match edges (directed forward: same patch → same patch in next frame)
    """
    # ====================== SPATIAL KNN EDGES ======================
    grid_size = int(N ** 0.5)
    ...
    # One-directional forward: frame0 → frame1, frame1 → frame2, ...
    temporal_src, temporal_dst = [], []
    for t in range(T - 1):
        src_local = torch.arange(N)                    # patch 0..255 in current frame
        dst_local = torch.arange(N)                    # same patch in next frame
        offset_curr = t * N
        offset_next = (t + 1) * N
        temporal_src.append(src_local + offset_curr)
        temporal_dst.append(dst_local + offset_next)
