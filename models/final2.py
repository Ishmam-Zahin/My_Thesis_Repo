...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, mlp_dim: int, dropout: float = 0.2):
        super().__init__()
...
class FusedModel(nn.Module):
    def __init__(
        ...
        # ====================== Stage 3: fusion via a stack of transformer blocks ======================
        # The same shared cls_token (already contextualized by Stage 1) is concatenated
        # with the cluster tokens and the whole sequence is refined by self-attention
        # across `num_fusion_layers` transformer blocks. No positional embedding is
        # added to the cluster tokens (clusters are a set, not an ordered sequence).
        self.fusion_blocks = nn.ModuleList([
            TransformerBlock(dim=feature_dim, num_heads=num_heads, mlp_dim=mlp_dim, dropout=dropout)
    ...
    def _encode_fused(self, x: torch.Tensor, branch_ablation: str | None = None):
        ...
        # ====================== Stage 3: fusion via transformer blocks (cls token + clusters) ======================
        fusion_seq = torch.cat([cls_vec, x_pooled], dim=1)   # [B, 1 + num_clusters, D], no positional embedding
        for block in self.fusion_blocks:
            fusion_seq = block(fusion_seq)

        fused_vec = fusion_seq[:, 0:1, :]     # take the cls token position back out
